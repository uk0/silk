...
func rectIcon(fg, bg color.RGBA) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		...
		m := imax(1, sz/8)
		for y := m; y < sz-m; y++ { for x := m; x < sz-m; x++ {
			if x==m||x==sz-m-1||y==m||y==sz-m-1 { img.SetRGBA(x, y, fg) } else { img.SetRGBA(x, y, bg) }
		}}; return img
	}
}
func circleIcon(bg, fg color.RGBA) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		...
		for y := 0; y < sz; y++ { for x := 0; x < sz; x++ {
			...
			if d <= r-1 { img.SetRGBA(x, y, bg) } else if d <= r { img.SetRGBA(x, y, fg) }
		}}; return img
	}
}
func crossIcon(c color.RGBA) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		img := image.NewRGBA(image.Rect(0, 0, sz, sz)); m := sz/4
		for i := 0; i < sz-2*m; i++ { for t := -1; t <= 1; t++ {
			if m+i+t >= 0 && m+i+t < sz { img.SetRGBA(m+i, m+i+t, c); img.SetRGBA(sz-1-m-i, m+i+t, c) }
		}}; return img
	}
}
func arrowIcon(c color.RGBA) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		img := image.NewRGBA(image.Rect(0, 0, sz, sz)); cx := sz*2/3
		for y := sz/4; y < sz*3/4; y++ { dy := y-sz/2; if dy<0{dy=-dy}
			for x := cx-(sz/4-dy); x <= cx && x < sz; x++ { if x >= 0 { img.SetRGBA(x, y, c) } }
		}; return img
	}
}
func lineIcon(c color.RGBA) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		img := image.NewRGBA(image.Rect(0, 0, sz, sz)); y := sz*3/4; t := imax(2, sz/8)
		for x := sz/4; x < sz*3/4; x++ { for dy := 0; dy < t; dy++ { img.SetRGBA(x, y+dy, c) } }
		return img
	}
}
func maxIcon(c color.RGBA) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		img := image.NewRGBA(image.Rect(0, 0, sz, sz)); m, t := sz/4, imax(2, sz/8)
		for x := m; x < sz-m; x++ { for dy := 0; dy < t; dy++ { img.SetRGBA(x, m+dy, c); img.SetRGBA(x, sz-m-1-dy, c) } }
		for y := m; y < sz-m; y++ { for dx := 0; dx < t; dx++ { img.SetRGBA(m+dx, y, c); img.SetRGBA(sz-m-1-dx, y, c) } }
		return img
	}
}
...
func checkboxIcon(checked bool) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		...
		for x := 0; x < sz; x++ { img.SetRGBA(x, 0, b); img.SetRGBA(x, sz-1, b) }
		for y := 0; y < sz; y++ { img.SetRGBA(0, y, b); img.SetRGBA(sz-1, y, b) }
		if checked { fg := color.RGBA{50, 150, 50, 255}
			for i := 0; i < sz/3; i++ { for t := -1; t <= 1; t++ {
				x1, y1 := sz/3-i, sz*2/3-i+t; x2, y2 := sz/3+i*2, sz*2/3-i*2+t
				if x1>=0&&y1>=0&&x1<sz&&y1<sz { img.SetRGBA(x1, y1, fg) }
				if x2>=0&&y2>=0&&x2<sz&&y2<sz { img.SetRGBA(x2, y2, fg) }
			}}
		}; return img
	}
}
func triRightIcon(c color.RGBA) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		img := image.NewRGBA(image.Rect(0, 0, sz, sz))
		for y := sz/4; y < sz*3/4; y++ { dy := y-sz/2; if dy<0{dy=-dy}
			for x := sz/3; x < sz/3+(sz/4-dy) && x < sz; x++ { img.SetRGBA(x, y, c) }
		}; return img
	}
}
func triDownIcon(c color.RGBA) func(int) *image.RGBA {
	return func(sz int) *image.RGBA {
		img := image.NewRGBA(image.Rect(0, 0, sz, sz))
		for x := sz/4; x < sz*3/4; x++ { dx := x-sz/2; if dx<0{dx=-dx}
			for y := sz/3; y < sz/3+(sz/4-dx) && y < sz; y++ { img.SetRGBA(x, y, c) }
		}; return img
	}
}
func themeImg(w, h int, c color.RGBA) *image.RGBA {
	...
	for x := 0; x < w; x++ { img.SetRGBA(x, 0, b); img.SetRGBA(x, h-1, b) }
	for y := 0; y < h; y++ { img.SetRGBA(0, y, b); img.SetRGBA(w-1, y, b) }
	return img
}
