...
func GenerateDesign(designPath, module string) (string, error) {
	...
	// Refuse a generated file this designer does not own. silkide writes the
	// same <stem>.silk.go name from a different generator (package ui +
	// BuildXxx() returning a decl.Node); overwriting it silently replaces a
	// function the user's hand-written code calls, and the report still says
	// success. Two generators sharing one output name is the real defect —
	// until they are reconciled, whoever wrote it first keeps it.
	if owner, err := generatedFileOwner(out.Generated); err != nil {
		return "", err
	} else if owner == ownerForeign {
		return "", fmt.Errorf("%s 由 silkide 生成, 设计器不会覆盖它", filepath.Base(out.Generated))
	}
	if _, err := scene.GenerateCodeFile(out.Generated, opts); err != nil {
		return "", err
	}
	return out.Generated, nil
}

// generatedFileOwner reads the marker at the top of an existing generated file.
// Absent file means nobody owns it yet.
type genOwner int

const (
	ownerNone genOwner = iota
	ownerDesigner
	ownerForeign
)

func generatedFileOwner(path string) (genOwner, error) {
	f, err := os.Open(path)
	if err != nil {
		if os.IsNotExist(err) {
			return ownerNone, nil
		}
		return ownerNone, err
	}
	defer f.Close()
	// The marker is the first line both generators emit; reading a fixed
	// prefix keeps this cheap even for a large file.
	buf := make([]byte, 256)
	n, _ := f.Read(buf)
	head := string(buf[:n])
	if strings.Contains(head, "generated by silkide") {
		return ownerForeign, nil
	}
	return ownerDesigner, nil
}
