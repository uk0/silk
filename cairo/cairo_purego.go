...
// Package cairo (silk_pure_go build) is a pure-Go re-implementation of
// the Cairo C-API surface silk uses, eliminating the libcairo CGO
// dependency. Build with `-tags silk_pure_go`.
//
// Scope: silk uses ~67 distinct cairo functions (see paint/painter_*.go).
// This file mirrors the Go signatures from cairo.go (the !silk_pure_go
// build) so silk/paint and the rest of the framework compile unchanged.
//
// Implementation status:
//   - Path construction (MoveTo / LineTo / Arc / ArcNegative / CurveTo /
//     Rectangle / Line / RoundRect / NewPath / ClosePath): real.
//     Each path segment is recorded as a sequence of points the
//     rasterizer consumes on Fill / Stroke.
//   - Fill / FillPreserve: real, uses golang.org/x/image/vector for
//     anti-aliased coverage rasterisation. Solid colours; gradient
//     patterns currently fall back to first-stop colour.
//   - Stroke / StrokePreserve: real for solid pens. Dash + cap + join
//     are recorded but rendered as solid (round-trip stub).
//   - Save / Restore + transform stack (Translate / Scale / Rotate /
//     Transform / SetMatrix / GetMatrix / ResetMatrix): real.
//   - Clip / ClipPreserve / ResetClip / ClipBounds: real, AABB-only —
//     clip path is approximated by its bounding rect, same simplification
//     as glui CairoCompat.
//   - Paint / PaintWithAlpha: real.
//   - Surface / Pattern / FontFace / ScaledFont / FontOptions / Glyph:
//     types defined; constructors return wrapped values; the most-used
//     methods are real, less-common ones (push/pop group, copy/append
//     paths, mask, hit-testing) are stubbed to no-op so callers can at
//     least compile and rasterise the common path.
//   - Text rendering: ShowGlyphs_hack / TextExtents / FontExtents use
//     the standard Go font (golang.org/x/image/font basicfont) as a
//     placeholder. Real font loading + opentype rasterisation is
//     follow-up work.
//
// What's NOT supported in this build:
//   - PDF / SVG / PostScript export surfaces (silk has its own pure-Go
//     svgexport / pdfexport packages — use those instead)
//   - cairo_xlib / cairo_quartz / cairo_win32 platform surfaces (silk
//     uses image surfaces only on the cairo path)
//   - Non-rect clip masks (clip path is approximated by AABB)
//   - Mesh / radial / linear gradient patterns at full fidelity
//
// Goal: silk compiles + renders basic widgets without libcairo. Cairo
// build remains the reference for byte-identical output.
package cairo
