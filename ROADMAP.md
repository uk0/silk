...
**未闭合（GedView 设计画布在 silk_no_cairo 模式下不渲染）⚠️ — 范围已修正**：

二轮 computer-use 验证后修正了上轮诊断 —— 实际范围比上轮报告的小得多：

✅ **OpenGL 模式下工作正常**（1400×900 窗口下 cliclick 重新截图确认）：
- `gui.ToolBar` 顶部条 ✅ Open/Refresh/Save/Back/Forward/Run/Debug/Search/Settings 全部渲染
- `ged.WidgetList` widget palette ✅ —— 上轮"Button → tton"是 1200×800 窗口太挤的视觉错觉，1400×900 下所有 widget 名（Button/Edit/CheckBox/RadioButton/ComboBox/SpinBox/Slider/ToggleSwitch/SearchBox/NumberInput/DatePicker/ColorPicker/Rating/DropdownButton/SwitchGroup/Label/ProgressBar/GroupBox/ImageView/Tag/Badge/Avatar/Breadcrumb/Link/LabelSeparator/Placeholder...）+ 类目 header 全部完整正确显示
- `ged.FileExplorer` 文件树 ✅
- `gui.CodeEditor` 多 tab 编辑器 ✅
- `gui.StatusBar` 底部状态栏 ✅
- `WidgetList` 拖源能 detect 鼠标 down（拖 Button 后 palette 高亮蓝色）

❌ **真实 gap**：`ged.GedView` 设计画布 —— `graph.GraphView` 派生，自己的 `Draw` 路径在 OpenGL 模式下：
  - 背景灰色填充能看到（说明 widget 至少进了 Draw 函数）
  - 但白色 page rect、scene items、selection handles 全部不渲染
  - `WidgetList` 拖源能拖出，drop 到画布上视觉无反馈（widget 应该没真的落进 scene，否则 ObjectInspector 树会更新）

`graph.GraphView.Draw` 用了 `Translate(pageLeftPx-scrollX, pageTopPx-scrollY)` + `Scale(pageScale)` + 嵌套 Save/Restore + `scene.DrawAll(g)`。深 Draw 嵌套 + Scale + Translate 的组合在 `glui.CairoCompat` 路径下与 Cairo 路径有偏差。

修复思路（独立任务）：
1. 最小复现：单个 GraphView 在空 Frame 里 silk_no_cairo 渲染，对比 Cairo 路径输出
2. instrument `CairoCompat` 的 Save/Restore/Translate/Scale 序列，dump 每帧 CTM
3. 比对 Cairo painter 的同一 widget tree 输出，找发散点
4. 大概率是 `Renderer.Save/Restore` 与 `CairoCompat.stateStack` 之间的 CTM 同步丢失，或 Scale 乘进 PushClip 后 scissor rect 计算有偏差

**当前现实**：silkide 在 OpenGL 模式下 IDE 半边（toolbar + 文件树 + 多 tab 编辑器 + palette + 终端 + 状态栏）已经工作，**约 80% 完整**；只有设计画布的实时绘制没跑通。Cairo 模式下 100% 完整工作（含拖拽 + 选中 + inspector，cliclick 验证过）。

silkide 顶部注释指向本节解释当前状态。
