...
func (this *cairoPainter) DrawIcon(ico Icon, fSize float64, grayed bool) {
	...
	case *icon:
		...
		sz := sub.img.Width()
		if sz != size {
			// Build a freshly-scaled cairo pattern for this draw. We
			// used to cache the pattern on subIcon.pat across draws,
			// but the cache field was Cairo-specific and complicated
			// the no-Cairo build split. Re-creating per draw is a
			// minor perf hit (icon draws are infrequent vs widget
			// repaints) and keeps subIcon backend-neutral.
			if cs, ok := sub.img.(*cairoSurface); ok {
