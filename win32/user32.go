...
func SetTimer(uElapse uint32) (timerId uintptr) {
...
func KillTimer(id uintptr) bool {
	ret, _, _ := procKillTimer.Call(0, id)
	return ret != 0
}

func GetQueueStatus(flags uint32) uint32 {
	ret, _, _ := procGetQueueStatus.Call(uintptr(flags))
	return uint32(ret)
}
...
func SystemParametersInfo(uiAction, uiParam uint32, pvParam uintptr, fWinIni uint32) bool {
...
func WindowFromPoint(x, y int32) HWND {
	// POINT拆为4字节一组, 即两个int32
	ret, _, _ := procWindowFromPoint.Call(uintptr(x), uintptr(y))
	return HWND(ret)
}

func CreateIconIndirect(iconinfo *ICONINFO) HICON {
	ret, _, _ := procCreateIconIndirect.Call(
...
func GetParent(hWnd HWND) HWND {
	// POINT拆为4字节一组, 即两个int32
	ret, _, _ := procWindowFromPoint.Call(uintptr(hWnd))
	return HWND(ret)
}

func GetWindowPlacement(hWnd HWND) (wp WINDOWPLACEMENT, ok bool) {
	ret, _, _ := procGetWindowPlacement.Call(uintptr(hWnd),
