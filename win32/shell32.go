...
func SHGetFolderPath(csid int) string {
	buf := make([]uint16, MAX_PATH)
	ret, _, _ := procSHGetFolderPathW.Call(0, // hwnd
		uintptr(csid),
		0, // token
		0, //SHGFP_TYPE_CURRENT = 0, SHGFP_TYPE_DEFAULT = 1
		uintptr(unsafe.Pointer(&buf[0])))
	if ret != 0 {
		return ""
	}
...
func SHCreateStdEnumFmtEtc(cfmt uint32, fetc *FORMATETC, ppEnumFormatEtc **IEnumFORMATETC) HRESULT {
	ret, _, _ := procSHCreateStdEnumFmtEtc.Call(
		uintptr(cfmt),
