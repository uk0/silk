...
var (
	...
	procInterlockedIncrement       = modkernel32.NewProc("InterlockedIncrement")
	procInterlockedDecrement       = modkernel32.NewProc("InterlockedDecrement")
)
...
func GetModuleFileName(hModule HINSTANCE) string {
	buf := make([]uint16, MAX_PATH)
	procGetModuleFileName.Call(
		uintptr(hModule),
...
func Sleep(dwMilliseconds uint32) {
	procSleep.Call(uintptr(dwMilliseconds))
}

func SetFileAttributes(filename string, dwFileAttributes uint32) bool {
	ret, _, _ := procSetFileAttributes.Call(
		uintptr(unsafe.Pointer(syscall.StringToUTF16Ptr(filename))),
		uintptr(dwFileAttributes))
	return ret != 0
}

func GlobalSize(hGlobal HGLOBAL) int {
	ret, _, _ := procGlobalSize.Call(uintptr(hGlobal))
	return int(ret)
}
...
func HeapAlloc(uFlags uint, dwBytes uint32) unsafe.Pointer {
	ret, _, _ := procHeapAlloc.Call(
		uintptr(unsafe.Pointer(processHeap)),
		uintptr(uFlags),
		uintptr(dwBytes))
...
func HeapFree(uFlags uint, p unsafe.Pointer) {
	ret, _, _ := procHeapFree.Call(
		uintptr(unsafe.Pointer(processHeap)),
		uintptr(uFlags),
		uintptr(p))

	if ret != 0 {
		panic("HeapFree failed")
	}
}

func InterlockedIncrement(p *int32) int32 {
	ret, _, _ := procInterlockedIncrement.Call(
		uintptr(unsafe.Pointer(p)))
	return int32(ret)
}

func InterlockedDecrement(p *int32) int32 {
	ret, _, _ := procInterlockedDecrement.Call(
		uintptr(unsafe.Pointer(p)))
	return int32(ret)
}
